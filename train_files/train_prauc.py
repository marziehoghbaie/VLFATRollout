...
import numpy as np
from test_files.test_roc import testROCTrain
...
def train_val(train_config,
    ...
    flag = True
    logger.info(f'filter_bkgrnd: {filter_bkgrnd}, rollout_warmup_epoch: {rollout_warmup_epoch}')
    for epoch in range(init_epoch, train_config['num_epochs']):
        ...
        accuracy, val_loss, balanced_acc, micro_roc_auc_ovr, val_acc = testROCTrain(val_loader, model, loss_fn, logger, device=device, phase='val')
        ...
        if filter_bkgrnd and epoch >= rollout_warmup_epoch:
            model.feature_extractor_spacial.filter_tokens = True
            if flag:
                logger.info(f'[INFO] Tokens are filtered in epoch {rollout_warmup_epoch}.')
                flag = False

        if VLFAT and epoch % 5 == 0:
