...
def testROC(loader, model, loss_fn, logger, phase, device='cuda', save_path=None, csv_file_name='results.csv'):
    model.eval()
    running_corrects = 0
    running_loss = 0
    trues = []
    ...
    # micro_roc_auc_ovr, aps = calculate_roc(y_score=y_score, y_test=trues, logger=logger)

    # print(len(all_sample_name),len(all_bscan_ids),len(trues),len(y_pred),len(y_score))
    df_results = pd.DataFrame(data={
        ...
        'prediction': y_pred,
        'y_score': y_score})
    df_results.to_csv(f'{save_path}/{csv_file_name}')
    ...
    return accuracy, loss, balanced_acc, 0, 0
...
def testROCTrain(loader, model, loss_fn, logger, phase, device='cuda'):
    model.eval()
    running_corrects = 0
    running_loss = 0
    trues = []
