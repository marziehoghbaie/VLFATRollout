...
def main(config_path):
    ...
    model = model_utils.create_model(model_config, model_layout, logger)
    VLFAT = False
    ...
    rollout_warmup_epoch = train_config['rollout_warmup_epoch']
    model = model.to(device)
    model.feature_extractor_spacial.filter_tokens = model_config['filter_tokens']
    model.feature_extractor_spacial.reserve_token_nums = [model_config['reserve_token_nums']]
    model.feature_extractor_spacial.discard_ratio = model_config['discard_ratio']
    model.feature_extractor_spacial.head_fusion = model_config['head_fusion']
    logger.info(f'[INFO] feature_extractor_spacial.filter_tokens: {model.feature_extractor_spacial.filter_tokens}')
    logger.info(f'[INFO] feature_extractor_spacial.reserve_layers: {model.feature_extractor_spacial.reserve_layers}')
    logger.info(
        f'[INFO] feature_extractor_spacial.reserve_token_nums: {model.feature_extractor_spacial.reserve_token_nums}')
    logger.info(f'[INFO] feature_extractor_spacial.head_fusion: {model.feature_extractor_spacial.head_fusion}')
    logger.info(f'[INFO] feature_extractor_spacial.discard_ratio: {model.feature_extractor_spacial.discard_ratio}')

    model = model.to(device)

    if train_config['train']:
        train_val(train_config, logger, model, optimizer, loss_fn, train_loader, val_loader, scheduler, ckpt_saver,
                  init_epoch, device, VLFAT, filter_bkgrnd=True, rollout_warmup_epoch=rollout_warmup_epoch)
    else:
