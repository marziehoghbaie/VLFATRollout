...
mean_3channel = [0.485, 0.456, 0.406]
std_3channel = [0.229, 0.224, 0.225]

mean_gry = [0.5]
std_gry = [0.5]


def transform_custom(images, image_size, gray_scale=True, resize=True):
    imgs = []
    ...
    for idx, image in enumerate(images):
        ...
        if gray_scale:
            image = T.Normalize(mean=mean_gry, std=std_gry)(image)
        else:
            image = T.Normalize(mean=mean_3channel, std=std_3channel)(image)
